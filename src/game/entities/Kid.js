...
export class Kid extends Entity {
  constructor(game, x, y, aggressionLevel = 1) {
    ...
    // Stun state (from Shush Wave)
    this.stunTimer = 0;
    this.stunAngle = 0;
  }
  
  stun(duration) {
    if (this.carriedBook) this.dropBook();
  ...
  update(deltaTime) {
    // Handle stunned state — freeze kid and skip normal AI
    if (this.stunTimer > 0) {
      this.stunTimer -= deltaTime;
      this.vx = 0;
      this.vy = 0;
      this.isMoving = false;
      this.stunAngle += deltaTime * 4;
      return;
    }
  ...
  render(ctx, interpolation) {
    ...
    if (this.stunTimer > 0) {
      ...
      for (let i = 0; i < starCount; i++) {
        const a = this.stunAngle + (i / starCount) * Math.PI * 2;
        const sx = this.getCenterX() + Math.cos(a) * orbitR;
        const sy = this.y - 14 + Math.sin(a) * 7;
        ctx.font = '13px Arial';
