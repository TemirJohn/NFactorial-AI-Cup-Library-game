...
export class PlayingState extends State {
  ...
  update(deltaTime) {
    ...
    // Handle shush wave activation (only when power-up is active)
    if (this.player && input.isActionPressed('shush')) {
      if (this.player.triggerShushWave()) {
        this.applyShushWave();
      }
    }
  ...
  render(renderer, interpolation) {
    ...
    for (const pu of this.powerUps) {
      if (this.isInViewport(pu, viewportX - padding, viewportY - padding,
                            viewportWidth + padding * 2, viewportHeight + padding * 2)) {
        renderer.addToLayer('entities', pu);
      }
    }
  ...
  renderUI(ctx) {
    ...
    // Off-screen power-up arrow indicators
    if (this.player && this.powerUps.length > 0) {
      const cam = this.game.camera;
      const vx = cam.getViewportX();
      const vy = cam.getViewportY();
      const vw = cam.viewportWidth / cam.zoom;
      const vh = cam.viewportHeight / cam.zoom;
      const margin = 40;

      for (const pu of this.powerUps) {
        const puScreenX = (pu.getCenterX() - vx) * cam.zoom;
        const puScreenY = (pu.getCenterY() - vy) * cam.zoom;
        const onScreen = puScreenX > margin && puScreenX < width - margin &&
                         puScreenY > margin && puScreenY < height - margin;
        if (onScreen) continue;

        // Clamp to screen edge and draw arrow
        const clampedX = Math.max(margin, Math.min(width - margin, puScreenX));
        const clampedY = Math.max(margin, Math.min(height - margin, puScreenY));
        const angle = Math.atan2(puScreenY - height / 2, puScreenX - width / 2);
  ...
  updatePowerUps(deltaTime) {
    ...
    const pickupR = 64; // generous pickup radius
  ...
  spawnPowerUp() {
    ...
    if (nearPlayer) {
      ...
      for (const angle of angles) {
        const tx = px + Math.cos(angle) * 120 - 18;
        const ty = py + Math.sin(angle) * 120 - 18;
        let safe = true;
        for (const shelf of this.shelves) {
          if (tx + 36 > shelf.x - 20 && tx < shelf.x + shelf.width + 20 &&
              ty + 36 > shelf.y - 20 && ty < shelf.y + shelf.height + 20) {
            safe = false; break;
          }
        }
        if (safe) { pos = { x: tx, y: ty }; break; }
      }
      if (!pos) pos = { x: px + 120, y: py };
    } else {
  ...
  findSafePowerUpPosition() {
    ...
    for (let attempt = 0; attempt < 60; attempt++) {
      const x = margin + Math.random() * (this.worldWidth - margin * 2 - pw);
      const y = margin + Math.random() * (this.worldHeight - margin * 2 - ph);
      let safe = true;
      for (const shelf of this.shelves) {
        if (x + pw > shelf.x - 30 && x < shelf.x + shelf.width + 30 &&
            y + ph > shelf.y - 30 && y < shelf.y + shelf.height + 30) {
          safe = false;
          break;
        }
      }
    ...
    // fallback: center aisle
    return { x: 800, y: 520 };
  }

  applyShushWave() {
    const player = this.player;
    const waveAngle = player.getShushWaveAngle();
    const halfAngle = player.shushWave.halfAngle;
    const range = player.shushWave.range;
    const stunDuration = player.shushWave.stunDuration;
    const px = player.getCenterX();
    const py = player.getCenterY();

    for (const kid of this.kids) {
      const dx = kid.getCenterX() - px;
      const dy = kid.getCenterY() - py;
      const dist = Math.sqrt(dx * dx + dy * dy);
      if (dist > range) continue;

      let angleDiff = Math.atan2(dy, dx) - waveAngle;
      // Normalize to [-π, π]
      while (angleDiff > Math.PI) angleDiff -= 2 * Math.PI;
      while (angleDiff < -Math.PI) angleDiff += 2 * Math.PI;

      if (Math.abs(angleDiff) <= halfAngle) {
        kid.stun(stunDuration);
        this.game.gameData.kidsRepelled++;
        // Award XP for each stunned kid
        this.awardXP(5);
      }
    }
  }
